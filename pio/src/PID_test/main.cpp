...
float KI = 0.005f;
float KD = 0.01f;
static const float INTEGRALE_MAX = 80.0f; // anti-windup clamp (PWM units)
...
void onTimer()
{
  ...
  // Velocity estimate with wrap-around handling + EMA low-pass filter
  float angle_delta = current_angle - last_angle;
  float raw_vel = angle_delta / DT;
  curr_vel = VEL_ALPHA * raw_vel + (1.0f - VEL_ALPHA) * curr_vel; // EMA
 
  // Shortest-path error
  float err = target_angle - current_angle;

    if (err > 0 && current_angle < 270){  // moving upward
        ff = FF_GAIN;
    }
    else {ff = 0;}
  ...
  float pid_out = pid + ff; // add feedforward term
  ...
  if (abs(err) < 0.5f && abs(v_setpoint) < 0.1f)
  {
    final_output = 0;
    integrale = 0.0f;
  }
...
//   Serial.println(ff);
  ...
  if (Serial.available() > 0)
  {
    ...
    else if (line.startsWith("FF:"))
    {
      ...
      {
        FF_GAIN = v;
        Serial.printf("FF_GAIN set to: %.4f\n", FF_GAIN);
      }
    }
  }
