#!/usr/bin/env python3
"""Action server for thermostat approach and lateral repositioning."""
...
class ThermActionServer(Node):
    """Drives forward, then strafes to reposition the thermostat."""
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info(
            f"[THERM] Goal received: thermostat starts on {side}"
        )
    ...
    def execute_callback(self, goal_handle):
        ...
        return self._result(True, "Thermostat reposition complete")
    ...
    def _run_step(
        ...
        while rclpy.ok() and time.time() - step_started < duration:
            ...
            progress = (
                elapsed_before_step + elapsed_in_step
            ) / total_duration
    ...
    @staticmethod
    def _twist(linear_x=0.0, linear_y=0.0):
        cmd = Twist()
