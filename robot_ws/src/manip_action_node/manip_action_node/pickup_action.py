#!/usr/bin/env python3
"""Action server that sends pickup slot commands to the manipulator node."""
...
from custom_msgs.action import Pickup
...
class PickupActionServer(Node):
    """Publishes pickup slot commands on the existing manip Blocks topic."""
    ...
    def __init__(self):
        ...
        self.declare_parameter("action_name", "pickup")
        self.declare_parameter("blocks_topic", "/manip_node/pickup")
        ...
        self.declare_parameter("command_timeout_sec", 45.0)
        ...
        self.get_logger().info(
            f"[PICKUP ACTION READY] action={action_name}, topic={blocks_topic}, status={status_topic}"
        )
    ...
    def goal_callback(self, goal_request):
        slots = list(goal_request.slots)
        if any(slot not in self.VALID_SLOT_STATES for slot in slots):
            self.get_logger().warn(
                f"[PICKUP] Rejecting invalid slots={slots}; expected 0 or 1"
            )
            return GoalResponse.REJECT

        present_count = sum(slots)
        if goal_request.count != present_count:
            self.get_logger().warn(
                f"[PICKUP] Rejecting count={goal_request.count}; "
                f"slots indicate {present_count} requested pickups"
            )
            return GoalResponse.REJECT

        self.get_logger().info(
            f"[PICKUP] Goal received: slots={slots}, count={goal_request.count}"
        )
        return GoalResponse.ACCEPT

    def cancel_callback(self, goal_handle):
        self.get_logger().info("[PICKUP] Cancel requested")
    ...
    def execute_callback(self, goal_handle):
        ...
        while time.time() < deadline:
            ...
            if seen_new_status and last_status == STATUS_PICKUP_DONE:
                goal_handle.succeed()
                self.get_logger().info(
                    f"[PICKUP] Manipulator completed slots={list(goal.slots)}, count={goal.count}"
                )
                return self._result(True, "Pickup completed", goal.count)

            if seen_new_status and last_status == STATUS_PICKUP_FAILED:
                goal_handle.abort()
                return self._result(False, "Manipulator reported pickup failure", goal.count)
